...
from conf import link_to_bd


def start_db():
    global connect_db, cursor_db
    connect_db = sqlite3.connect(link_to_bd)
    cursor_db = sqlite3.cursor()

    if connect_db:
        print("Database is work.")

    connect_db.execute("""CREATE TABLE IF NOT EXIST db(
...
def add_info_to_db(user_id: int, user_nickname: str, user_text: str, predict: bool):
    cursor_db.execute("INSERT INTO db VALUES (?, ?, ?, ?)", (user_id, user_nickname, user_text, predict))
    connect_db.commit()
